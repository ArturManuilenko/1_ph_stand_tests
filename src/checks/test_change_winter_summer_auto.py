...
""" Проверка перехода на зимнее/летнее время автоматически.
...
def test_change_winter_summer_auto(
    ...
    logger.info('Проверка перехода зима/лето  автоматически')
    relay_block.all_off()
    relay_block.ave_on()
    wait.wait(wakeup)
    try:
        ...
        wait.wait(wakeup)
        tool.execute("-os 3=[2] -a 2")  # переход автоматический [2]
        ...
        tool.set_time("31.03.2018 02:59:40")
